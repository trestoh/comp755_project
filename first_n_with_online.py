...
fdr_file = open("online_first_30k_20fdr_5_batch_fdr.txt", "a")
...
while (count < len(data)):
    ...
    if current_scan != last_scan:
        ...
        if (current_scan % batch_size) == (batch_size - 1):
            training_set = batch_hits
            #print (batch_hits)
            #print (decoy_cands)
            np.random.shuffle(decoy_cands)
            #print (decoy_cands)
            for index in range(0, len(batch_hits)):
                training_set.append(decoy_cands[index])

            #print(len(batch_hits))
            ...
            if len(training_set) > 0:
                ...
                sgd_clf.partial_fit(train_set, train_labels)
...
with open("online_first_30k_20fdr_5_batch.txt", "a") as myfile:
