...
classes = [-1,1]
sgd_clf = linear_model.SGDClassifier(loss="hinge", warm_start=True, max_iter=1000, tol=1e-3, shuffle=True, penalty='none')
...
fdr_file = open("online_long_fdr.txt", "a")
...
while (count < len(data)):
    ...
    if current_scan == last_scan:
        if int(pre_trans_data[count][1]) == -1:
            decoy_cands.append(count)

    if current_scan != last_scan:
        #
        # Batch Additional Processing Section
        #
        if (current_scan % batch_size) == (batch_size - 1):
            training_set = []
            ...
            inner_count = 0
            inner_scan = int(pre_trans_data[inner_count][0])
            inner_last_scan = 0
            ...
            train_set = data[training_set, :]
            train_labels = labels[training_set]
            ...
            while (inner_count < count):
                inner_scan = int(pre_trans_data[inner_count][0])
                #uncomment and tab for less consideration
                if inner_scan != inner_last_scan:
                    inner_last_scan = inner_scan
                    pred = sgd_clf.decision_function(data[inner_count, :].reshape(1,-1))
                    #print (pred[0])
                    #print(int(pre_trans_data[count][1]), pred[0])
                    predictions.append((int(pre_trans_data[inner_count][1]), pred[0]))
...
with open("big_online_first_30k_20fdr_1000_batch.txt", "a") as myfile:
